...
export default function OrdersPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      <Link href="/account" className="inline-flex items-center gap-1.5 text-sm text-ig-green hover:underline mb-4">
      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          ...
          <Link href="/products">
            <button className="bg-ig-green hover:bg-ig-green/90 text-white px-6 py-2 rounded-lg font-medium">Shop Now</button>
          </Link>
        </div>
      ) : (
        <div className="space-y-4">
          {orders.map((order) => {
            ...
            return (
              <div key={order.id} className="bg-white border border-border rounded-lg p-5 hover:shadow-sm transition-shadow">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <p className="text-sm font-semibold text-foreground">Order #{order.order_number}</p>
                    <p className="text-xs text-muted-foreground mt-0.5">{new Date(order.created_at).toLocaleDateString("en-KE", { year: "numeric", month: "long", day: "numeric" })}</p>
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <p className="text-sm text-muted-foreground">{order.item_count} item{order.item_count !== 1 ? "s" : ""}</p>
